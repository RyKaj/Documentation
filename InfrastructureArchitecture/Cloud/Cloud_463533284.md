...
![](https://www.sam-solutions.com/blog/wp-content/uploads/2017/08/a-cloud-provider@2x.png){height="250"}
...
should be selected, aswell as the implications on the application.\
[Cloud application
...
### Solution[Permalink](http://www.cloudcomputingpatterns.org/community_cloud/#solution "Permalink") {#Cloud-SolutionPermalink}
...
| .confluence-external-resource}   | security and trust               |
|                               | requirements?*                   |
+----------------------------------+----------------------------------+
...
Random workloads are a generalization of [Periodic
...
### Context[Permalink](http://www.cloudcomputingpatterns.org/hypervisor/#context "Permalink") {#Cloud-ContextPermalink.17}
...
Distributed cloud applications often need to handle large data elements,
...
Using multiple replicas of data is vital to ensure resiliency of a
...
### Solution[Permalink](http://www.cloudcomputingpatterns.org/key_value_storage/#solution "Permalink") {#Cloud-SolutionPermalink.22}
...
### Solution[Permalink](http://www.cloudcomputingpatterns.org/relational_database/#solution "Permalink") {#Cloud-SolutionPermalink.23}
...
### Solution[Permalink](http://www.cloudcomputingpatterns.org/strict_consistency/#solution "Permalink") {#Cloud-SolutionPermalink.24}
...
Sometimes, message duplicity can be coped with by the application using
a [Message-oriented
...
### Solution[Permalink](http://www.cloudcomputingpatterns.org/exactly_once_delivery/#solution "Permalink") {#Cloud-SolutionPermalink.26}
...
Communication partners exchange information asynchronously using
...
The interface of a restricted application component is duplicated to
...
Asynchronous processing requests are accepted at all times, but stored
...
### Solution[Permalink](http://www.cloudcomputingpatterns.org/data_abstractor/#solution "Permalink") {#Cloud-SolutionPermalink.33}
...
### Solution[Permalink](http://www.cloudcomputingpatterns.org/data_access_component/#solution "Permalink") {#Cloud-SolutionPermalink.34}
...
### Solution[Permalink](http://www.cloudcomputingpatterns.org/dedicated_component/#solution "Permalink") {#Cloud-SolutionPermalink.35}
...
### Solution[Permalink](http://www.cloudcomputingpatterns.org/distributed_application/#solution "Permalink") {#Cloud-SolutionPermalink.36}
...
### Context[Permalink](http://www.cloudcomputingpatterns.org/message_mover/#context "Permalink") {#Cloud-ContextPermalink.40}
...
| ![Elasticity Management          | to which application components  |
| Process](http://www.clo          | are scaled-out be adjusted       |
| udcomputingpatterns.org/icons/el | efficiently to the currently     |
| asticity_management_process_icon | experienced workload and         |
| .png){.confluence-embedded-image | anticipated future workload?*    |
| .confluence-external-resource}   |                                  |
|                               |                                  |
+----------------------------------+----------------------------------+


### Context[Permalink](http://www.cloudcomputingpatterns.org/elasticity_management_process/#context "Permalink") {#Cloud-ContextPermalink.54}

A [Distributed
Application](http://www.cloudcomputingpatterns.org/distributed_application/) uses [Elasticity
Managers](http://www.cloudcomputingpatterns.org/elasticity_manager/), [Elastic
Queues](http://www.cloudcomputingpatterns.org/elastic_queue/),
or [Elastic Load
Balancers](http://www.cloudcomputingpatterns.org/elastic_load_balancer/) to
ensure an elastic scaling of application components. To handle this task
adequately, the current resource demand has to be obtained automatically
from the application and has to be reflected in provisioning and
decommissioning of cloud resources.

### Solution[Permalink](http://www.cloudcomputingpatterns.org/elasticity_management_process/#solution "Permalink") {#Cloud-SolutionPermalink.54}

An Elasticity Management Process analyzes the utilization of application
component instances in intervals, when a system manager requests it, or
if certain conditions are observed by the monitoring component. Based on
this information, the current workload of the application is computed
and reflected by adjusting used resources.

![Elasticity Management
Process](http://www.cloudcomputingpatterns.org/sketches/elasticity_management_process_sketch.png){.confluence-embedded-image
.confluence-external-resource}



 {#expander-987173825 .expand-container}
 {#expander-control-987173825 .expand-control}
Feature Flag Management Process


 {#expander-content-987173825 .expand-content}
If the cloud cannot provide required resources in time, the features
provided by application components are degraded gracefully to replace or
disable unimportant ones in order to keep vital features operational.

 {.table-wrap}
+----------------------------------+----------------------------------+
|  {.content-wrapper}           | *How can the performance of an   |
| ![Feature Flag Management        | application degrade gracefully,  |
| Process](http://www.cloud        | if the experienced workload      |
| computingpatterns.org/icons/feat | increases but additional cloud   |
| ure_flag_management_process_icon | resources are unavailable or     |
| .png){.confluence-embedded-image | take too long to provision?*     |
| .confluence-external-resource}   |                                  |
|                               |                                  |
+----------------------------------+----------------------------------+


### Context[Permalink](http://www.cloudcomputingpatterns.org/feature_flag_management_process/#context "Permalink") {#Cloud-ContextPermalink.55}

While the elasticity of clouds generally allows a tight alignment of
resource numbers to the experienced workload, the time it takes to
provision new resources remains as a limiting factor. If the workload
increases too drastically, it may take too long to provision new
resources. Furthermore, cloud providers often do not guarantee concrete
provisioning times.

### Solution[Permalink](http://www.cloudcomputingpatterns.org/feature_flag_management_process/#solution "Permalink") {#Cloud-SolutionPermalink.55}

Less important application functionality provided by application
component instances is disabled or replaced with a less demanding
implementation, if the cloud provider cannot fulfill current workload
demands. When resources can eventually be provisioned again, the
application components return to normal operation.

![Feature Flag Management
Process](http://www.cloudcomputingpatterns.org/sketches/feature_flag_management_process_sketch.png){.confluence-embedded-image
.confluence-external-resource}



 {#expander-154873184 .expand-container}
 {#expander-control-154873184 .expand-control}
Managed Configuration


 {#expander-content-154873184 .expand-content}
Scaled-out application components should use a centrally stored
configuration to provide a unified behavior that can be adjusted
simultaneously.

 {.table-wrap}
+----------------------------------+----------------------------------+
|  {.content-wrapper}           | *How can the configuration of    |
| ![Managed                        | scaled out application component |
| Configuration](http:/            | instances be controlled in a     |
| /www.cloudcomputingpatterns.org/ | coordinated fashion?*            |
| icons/managed_configuration_icon |                                  |
| .png){.confluence-embedded-image |                                  |
| .confluence-external-resource}   |                                  |
|                               |                                  |
+----------------------------------+----------------------------------+
...
### Context[Permalink](http://www.cloudcomputingpatterns.org/managed_configuration/#context "Permalink") {#Cloud-ContextPermalink.56}

Application components of a [Distributed
Application](http://www.cloudcomputingpatterns.org/distributed_application/) often
have configuration parameters. Storing configuration information
together with the application component implementation can be
unpractical as it results in more overhead in case of configuration
changes. Each running instance of the application component must be
updated separately. Component images stored in [Elastic
Infrastructures](http://www.cloudcomputingpatterns.org/elastic_infrastructure/) or [Elastic
Platforms](http://www.cloudcomputingpatterns.org/elastic_platform/) also
have to be updated upon configuration change.

### Solution[Permalink](http://www.cloudcomputingpatterns.org/managed_configuration/#solution "Permalink") {#Cloud-SolutionPermalink.56}

Configurations are stored in a central [Storage
Offering](http://www.cloudcomputingpatterns.org/cloud_offerings/#storage_offerings),
commonly, a [Relational
Database](http://www.cloudcomputingpatterns.org/relational_database/), [Key-Value
Storage](http://www.cloudcomputingpatterns.org/key_value_storage/),
or [Blob
Storage](http://www.cloudcomputingpatterns.org/blob_storage/) from
where it is accessed by all running component instances either by
accessing the storage periodically or by sending messages to the
components.

![Managed
Configuration](http://www.cloudcomputingpatterns.org/sketches/managed_configuration_sketch.png){.confluence-embedded-image
.confluence-external-resource}



 {#expander-932778836 .expand-container}
 {#expander-control-932778836 .expand-control}
Provider Adapter


 {#expander-content-932778836 .expand-content}
Provider interfaces are encapsulated and mapped to unified interfaces
used in applications to separate concerns of interactions with the
provider from application functionality.

 {.table-wrap}
+----------------------------------+----------------------------------+
|  {.content-wrapper}           | *How can the dependencies of an  |
| ![Provider                       | application component on a       |
| Adapter](h                       | provider-specific interface be   |
| ttp://www.cloudcomputingpatterns | managed?*                        |
| .org/icons/provider_adapter_icon |                                  |
| .png){.confluence-embedded-image |                                  |
| .confluence-external-resource}   |                                  |
|                               |                                  |
+----------------------------------+----------------------------------+
...
### Context[Permalink](http://www.cloudcomputingpatterns.org/provider_adapter/#context "Permalink") {#Cloud-ContextPermalink.57}

Cloud providers offer many interfaces that can be used in application
components of a [Distributed
Application](http://www.cloudcomputingpatterns.org/distributed_application/).
If a component directly interacts with these interfaces, its
implementation becomes strongly interleaved with the specific functions
offered and the protocols used.

### Solution[Permalink](http://www.cloudcomputingpatterns.org/provider_adapter/#solution "Permalink") {#Cloud-SolutionPermalink.57}

The Provider Adapter encapsulates all provider-specific implementations
required for authentication, data formatting etc. in an abstract
interface. The Provider Adapter , thus, ensures separation of concerns
between application components accessing provider functionality and
application components providing application functionality. It may also
offer synchronous provider-interfaces to be accessed asynchronously via
messages and vice versa.

![Provider
Adapter](http://www.cloudcomputingpatterns.org/sketches/provider_adapter_sketch_1.png){.confluence-embedded-image
.confluence-external-resource}

![Provider
Adapter](http://www.cloudcomputingpatterns.org/sketches/provider_adapter_sketch_2.png){.confluence-embedded-image
.confluence-external-resource}



 {#expander-1237768075 .expand-container}
 {#expander-control-1237768075 .expand-control}
Resiliency Management Process


 {#expander-content-1237768075 .expand-content}
Application components are checked for failures and replaced
automatically without human intervention.

 {.table-wrap}
+----------------------------------+----------------------------------+
|  {.content-wrapper}           | *How can the overall             |
| ![Resiliency Management          | availability of an application   |
| Process](http://www.clo          | be ensured automatically even if |
| udcomputingpatterns.org/icons/re | individual application component |
| siliency_management_process_icon | instances fail?*                 |
| .png){.confluence-embedded-image |                                  |
| .confluence-external-resource}   |                                  |
|                               |                                  |
+----------------------------------+----------------------------------+
...
### Context[Permalink](http://www.cloudcomputingpatterns.org/resiliency_management_process/#context "Permalink") {#Cloud-ContextPermalink.58}

A [Watchdog](http://www.cloudcomputingpatterns.org/watchdog/) allows
to monitor application components and react to failures. To handle this
task, the component functionality must be verified and failing
components must be replaced with newly provisioned components in a
coordinated fashion.

### Solution[Permalink](http://www.cloudcomputingpatterns.org/resiliency_management_process/#solution "Permalink") {#Cloud-SolutionPermalink.58}

This process is triggered by the monitoring functionality or by
the [Watchdog](http://www.cloudcomputingpatterns.org/watchdog/) if
it detects a component failure. Additionally, the Resiliency Management
Process periodically verifies application component health. If a failure
is detected, the faulty application component instance is decommissioned
and replaced by a newly provisioned instance.

![Resiliency Management
Process](http://www.cloudcomputingpatterns.org/sketches/resiliency_management_process_sketch.png){.confluence-embedded-image
.confluence-external-resource}



 {#expander-1356690362 .expand-container}
 {#expander-control-1356690362 .expand-control}
Standby Pooling Process


 {#expander-content-1356690362 .expand-content}
Application component instances should be kept on standby to increase
provisioning speed and utilize billing time-slots efficiently.

 {.table-wrap}
+----------------------------------+----------------------------------+
|  {.content-wrapper}           | *How can defined provisioning    |
| ![Standby Pooling                | times for application component  |
| Process](http://w                | instances be ensured while       |
| ww.cloudcomputingpatterns.org/ic | utilizing pay-per-use resources  |
| ons/standby_pooling_process_icon | in an optimal fashion?*          |
| .png){.confluence-embedded-image |                                  |
| .confluence-external-resource}   |                                  |
|                               |                                  |
+----------------------------------+----------------------------------+
...
### Context[Permalink](http://www.cloudcomputingpatterns.org/standby_pooling_process/#context "Permalink") {#Cloud-ContextPermalink.59}

Even though application component instances may be provisioned and
decommissioned dynamically, it usually requires some time to actually
provision and decommission them. If a cloud application, however,
experiences drastic and quick workload changes, these provisioning times
may limit its capability to obtain the required resources quickly
enough. Decommissioning of component instances immediately when no
longer needed may also be ineffective, if cloud resources are charged
for fixed time-slots.

### Solution[Permalink](http://www.cloudcomputingpatterns.org/standby_pooling_process/#solution "Permalink") {#Cloud-SolutionPermalink.59}

Instead of decommissioning application component instances instantly
when they are unused, they are assigned to a standby list They are
decommissioned only when the time-slot they have been paid for has been
utilized and they are still not needed. The standby list may always
contain a certain number of component instances to ensure timely
provisioning.

![Standby Pooling
Process](http://www.cloudcomputingpatterns.org/sketches/standby_pooling_process_sketch_1.png){.confluence-embedded-image
.confluence-external-resource}

![Standby Pooling
Process](http://www.cloudcomputingpatterns.org/sketches/standby_pooling_process_sketch_2.png){.confluence-embedded-image
.confluence-external-resource}



 {#expander-14676055 .expand-container}
 {#expander-control-14676055 .expand-control}
Update Transition Process


 {#expander-content-14676055 .expand-content}
...
+----------------------------------+----------------------------------+
|  {.content-wrapper}           | *How can application components  |
| ![Update Transition              | of a Distributed Application be  |
| Process](http://www              | updated seamlessly?*             |
| .cloudcomputingpatterns.org/icon |                                  |
| s/update_transition_process_icon |                                  |
| .png){.confluence-embedded-image |                                  |
| .confluence-external-resource}   |                                  |
|                               |                                  |
+----------------------------------+----------------------------------+
...
### Context[Permalink](http://www.cloudcomputingpatterns.org/update_transition_process/#context "Permalink") {#Cloud-ContextPermalink.60}

During the runtime of a [Distributed
Application](http://www.cloudcomputingpatterns.org/distributed_application/),
new versions of used middleware, operating systems, or application
components may become available. A seamless switch from the old to the
new version of application components shall be enabled.

### Solution[Permalink](http://www.cloudcomputingpatterns.org/update_transition_process/#solution "Permalink") {#Cloud-SolutionPermalink.60}

The new component version is created. Additional application component
instances of the new version are provisioned. These components are
executed simultaneously with the application components of the old
version. If necessary, load balancing is then switched to the component
instances of the new version. If the application components access a
queue, this step is unnecessary. Finally, the old application component
instances are decommissioned.

![Update Transition
Process](http://www.cloudcomputingpatterns.org/sketches/update_transition_process_sketch.png){.confluence-embedded-image
.confluence-external-resource}



 {#expander-226652981 .expand-container}
 {#expander-control-226652981 .expand-control}
Watchdog


 {#expander-content-226652981 .expand-content}
Applications cope with failures automatically by monitoring and
replacing application component instances if the provider-assured
availability is insufficient.

 {.table-wrap}
+----------------------------------+----------------------------------+
|  {.content-wrapper}           | *How can applications            |
| ![Wat                            | automatically detect failing     |
| chdog](http://www.cloudcomputing | application components and       |
| patterns.org/icons/watchdog_icon | handle their replacement?*       |
| .png){.confluence-embedded-image |                                  |
| .confluence-external-resource}   |                                  |
|                               |                                  |
+----------------------------------+----------------------------------+
...
### Context[Permalink](http://www.cloudcomputingpatterns.org/watchdog/#context "Permalink") {#Cloud-ContextPermalink.61}

If a [Distributed
Application](http://www.cloudcomputingpatterns.org/distributed_application/) is
comprised of many application components it is dependent on the
availability of all component instances. To enable high availability
under such conditions, applications have to rely on redundant
application component instances and the failure of these instances has
to be detected and coped with automatically.

### Solution[Permalink](http://www.cloudcomputingpatterns.org/watchdog/#solution "Permalink") {#Cloud-SolutionPermalink.61}

Individual application components rely on external state information by
implementing the [Stateless
Component](http://www.cloudcomputingpatterns.org/stateless_component/) pattern.
Components are scaled out and multiple instances of them are deployed to
redundant resources. The component instances are monitored by a separate
Watchdog component and replaced in case of failures.

![Watchdog](http://www.cloudcomputingpatterns.org/sketches/watchdog_sketch.png){.confluence-embedded-image
.confluence-external-resource}
...
**Cloud Applications**
 ...
 {.panelContent}
 {#expander-1018799854 .expand-container}
 {#expander-control-1018799854 .expand-control}
Content Distribution Network


 {#expander-content-1018799854 .expand-content}
Applications component instances and data handled by them are globally
distributed to meet the access performance required by a global user
group.

 {.table-wrap}
+----------------------------------+----------------------------------+
|  {.content-wrapper}           | *How can timely access to an     |
| ![Content Distribution           | application be ensured for a     |
| Network](http://www.cl           | globally distributed user        |
| oudcomputingpatterns.org/icons/c | group?*                          |
| ontent_distribution_network_icon |                                  |
| .png){.confluence-embedded-image |                                  |
| .confluence-external-resource}   |                                  |
|                               |                                  |
+----------------------------------+----------------------------------+
...
### Context[Permalink](http://www.cloudcomputingpatterns.org/content_distribution_network/#context "Permalink") {#Cloud-ContextPermalink.62}

If the application provides multimedia content to users, for example,
streamed videos and music the amount of data to be served increases
drastically. If such multimedia content is located too far from the user
accessing it, the communication delay of the distribution network may
hinder the timely access to data. Therefore, storing content in only one
centralized location, i.e., one cloud or data center is unfeasible.

### Solution[Permalink](http://www.cloudcomputingpatterns.org/content_distribution_network/#solution "Permalink") {#Cloud-SolutionPermalink.62}

Content replicas are established in different physical locations of one
or multiple clouds. During this distribution of replicas, the topology
of distribution networks is considered to ensure locality for all users.
Replicas are updated from a central location.

![Content Distribution
Network](http://www.cloudcomputingpatterns.org/sketches/content_distribution_network_sketch.png){.confluence-embedded-image
.confluence-external-resource}



 {#expander-1579710379 .expand-container}
 {#expander-control-1579710379 .expand-control}
Hybrid Application Functions


 {#expander-content-1579710379 .expand-content}
Some application functionality provided by user interfaces, processing,
and data handling is experiencing varying workload and is hosted in an
elastic cloud while other application functionality of the same type is
hosted in a static environment.

 {.table-wrap}
+----------------------------------+----------------------------------+
|  {.content-wrapper}           | *How can arbitrary functionality |
| ![Hybrid Application             | of an application be distributed |
| Functions](http://www.cl         | among static data centers and    |
| oudcomputingpatterns.org/icons/h | elastic clouds best matching its |
| ybrid_application_functions_icon | requirements?*                   |
| .png){.confluence-embedded-image |                                  |
| .confluence-external-resource}   |                                  |
|                               |                                  |
+----------------------------------+----------------------------------+
...
### Context[Permalink](http://www.cloudcomputingpatterns.org/hybrid_application_functions/#context "Permalink") {#Cloud-ContextPermalink.63}

Application components comprising a [Distributed
Application](http://www.cloudcomputingpatterns.org/distributed_application/) experience
varying workloads on all layers of the application stack: [User
Interface
Components](http://www.cloudcomputingpatterns.org/user_interface_component/), [Processing
Components](http://www.cloudcomputingpatterns.org/processing_component/),
and [Data Access
Components](http://www.cloudcomputingpatterns.org/data_access_component/).
All of these components provide functionality to the user group of the
application, but this user group accesses functionality differently. In
addition to the workload requirements other issues, such as legal and
corporate regulations or requirements on security, privacy, and trust
may limit the environments to which an application component may be
provisioned.

### Solution[Permalink](http://www.cloudcomputingpatterns.org/hybrid_application_functions/#solution "Permalink") {#Cloud-SolutionPermalink.63}

Application components are grouped regarding similar requirements and
are deployed into best fitting environments. Interdependencies between
the components are reduced by exchanging data using asynchronous
messaging to ensure [Loose
Coupling](http://www.cloudcomputingpatterns.org/loose_coupling/).
Depending on the accessed function, a load balancer redirects user
accesses to the different environments seamlessly.

![Hybrid Application
Functions](http://www.cloudcomputingpatterns.org/sketches/hybrid_application_functions_sketch.png){.confluence-embedded-image
.confluence-external-resource}



 {#expander-381695395 .expand-container}
 {#expander-control-381695395 .expand-control}
Hybrid Backend


 {#expander-content-381695395 .expand-content}
Backend functionality comprised of data intensive processing and data
storage is experiencing varying workloads and is hosted in an elastic
cloud while the rest of an application is hosted in a static data
center.

 {.table-wrap}
+----------------------------------+----------------------------------+
|  {.content-wrapper}           | *How can Processing Components   |
| ![Hybrid                         | that experience varying workload |
| Backend]                         | and need access to large amounts |
| (http://www.cloudcomputingpatter | of data be hosted in an elastic  |
| ns.org/icons/hybrid_backend_icon | environment while the remainder  |
| .png){.confluence-embedded-image | of the application is hosted in  |
| .confluence-external-resource}   | a static environment?*           |
|                               |                                  |
+----------------------------------+----------------------------------+


### Context[Permalink](http://www.cloudcomputingpatterns.org/hybrid_backend/#context "Permalink") {#Cloud-ContextPermalink.64}

A [Distributed
Application](http://www.cloudcomputingpatterns.org/distributed_application/) provides
processing functionality that experiences varying workload behavior.
Mainly, [Static
Workload](http://www.cloudcomputingpatterns.org/static_workload/) has
to be handled, but some Processing Components experience [Periodic
Workload](http://www.cloudcomputingpatterns.org/periodic_workload/), [Unpredictable
Workload](http://www.cloudcomputingpatterns.org/unpredictable_workload/),
or [Continuously Changing
Workload](http://www.cloudcomputingpatterns.org/continuously_changing_workload/).
Application components providing the respective processing functionality
experiencing varying workload should, therefore, be hosted in an elastic
environment. However, these components have to access large amounts of
data during their execution making them highly dependent on the
availability and the timely access to such data.

### Solution[Permalink](http://www.cloudcomputingpatterns.org/hybrid_backend/#solution "Permalink") {#Cloud-SolutionPermalink.64}

The Processing Components experiencing varying workloads are hosted in
an elastic cloud together with the data accessed during their operation.
Processing Components in the elastic cloud are triggered from the static
environment through asynchronous messages exchanged via message queues
provided by a [Message-oriented
Middleware](http://www.cloudcomputingpatterns.org/message_oriented_middleware/).
A [Data Access
Component](http://www.cloudcomputingpatterns.org/data_access_component/) in
the static environment ensures that data required by elastic Processing
Components is stored in Storage Offerings The location where this data
is stored may then be passed to the elastic Processing Components during
their enactment via messages. Data that is not required by the backend
functionality may still be stored in [Stateful
Components](http://www.cloudcomputingpatterns.org/stateful_component/) hosted
in the static data center.

![Hybrid
Backend](http://www.cloudcomputingpatterns.org/sketches/hybrid_backend_sketch.png){.confluence-embedded-image
.confluence-external-resource}



 {#expander-573642273 .expand-container}
 {#expander-control-573642273 .expand-control}
Hybrid Backup


 {#expander-content-573642273 .expand-content}
Data is periodically extracted from an application to be archived in an
elastic cloud for disaster recovery purposes.

 {.table-wrap}
+----------------------------------+----------------------------------+
|  {.content-wrapper}           | *How can data be archived in a   |
| ![Hybrid                         | remote environment while the     |
| Backup                           | remainder of the application is  |
| ](http://www.cloudcomputingpatte | hosted in a static environment?* |
| rns.org/icons/hybrid_backup_icon |                                  |
| .png){.confluence-embedded-image |                                  |
| .confluence-external-resource}   |                                  |
|                               |                                  |
+----------------------------------+----------------------------------+
...
### Context[Permalink](http://www.cloudcomputingpatterns.org/hybrid_backup/#context "Permalink") {#Cloud-ContextPermalink.65}

Many applications are used by small and medium businesses which do not
have the required IT skills to host and maintain their own highly
available infrastructure. Especially, requirements regarding business
resiliency -- the ability to recover from an error -- and business
continuity -- the ability to operate during an error -- are challenging.
Furthermore, there are laws and regulations making businesses liable to
archive data and keep it accessible for audits, often over very long
periods of time.

### Solution[Permalink](http://www.cloudcomputingpatterns.org/hybrid_backup/#solution "Permalink") {#Cloud-SolutionPermalink.65}

A [Distributed
Application](http://www.cloudcomputingpatterns.org/distributed_application/) is
hosted in a local static environment of the company. Data handled
by [Stateful
Components](http://www.cloudcomputingpatterns.org/stateful_component/) is
periodically extracted and replicated to a cloud storage offering.

![Hybrid
Backup](http://www.cloudcomputingpatterns.org/sketches/hybrid_backup_sketch.png){.confluence-embedded-image
.confluence-external-resource}



 {#expander-2067748698 .expand-container}
 {#expander-control-2067748698 .expand-control}
Hybrid Data


 {#expander-content-2067748698 .expand-content}
Data of varying size is hosted in an elastic cloud while the remainder
of an application resides in a static environment.

 {.table-wrap}
+----------------------------------+----------------------------------+
|  {.content-wrapper}           | *How can a data handling         |
| ![Hybrid                         | functionality that experiences   |
| Da                               | varying workload be hosted in an |
| ta](http://www.cloudcomputingpat | elastic cloud while the rest of  |
| terns.org/icons/hybrid_data_icon | the application is located in a  |
| .png){.confluence-embedded-image | static data center?*             |
| .confluence-external-resource}   |                                  |
|                               |                                  |
+----------------------------------+----------------------------------+


### Context[Permalink](http://www.cloudcomputingpatterns.org/hybrid_data/#context "Permalink") {#Cloud-ContextPermalink.66}

A [Distributed
Application](http://www.cloudcomputingpatterns.org/distributed_application/) handles
data whose size varies drastically over time. Large amounts of data may,
thus, be generated periodically and are then deleted again, may increase
and decrease randomly, or may display a general increase or decrease
over time. Especially, during these changes, the user number and their
accesses to the application can be static resulting in [Static
Workload](http://www.cloudcomputingpatterns.org/static_worklaod/) on
the remainder of the application components.

### Solution[Permalink](http://www.cloudcomputingpatterns.org/hybrid_data/#solution "Permalink") {#Cloud-SolutionPermalink.66}

Data whose varying size makes it unsuitable for hosting in a static
environment is handled by Storage Offerings in an elastic cloud. At this
location data is either accessed by [Data Access
Components](http://www.cloudcomputingpatterns.org/data_access_component/) that
are hosted in the static environment or by Data Access Components hosted
in the elastic environment.

![Hybrid
Data](http://www.cloudcomputingpatterns.org/sketches/hybrid_data_sketch.png){.confluence-embedded-image
.confluence-external-resource}



 {#expander-1046196777 .expand-container}
 {#expander-control-1046196777 .expand-control}
Hybrid Development Environment


 {#expander-content-1046196777 .expand-content}
...
environment where new applications can be developed and tested.
 ...
 {.table-wrap}
+-----------------------------------+-----------------------------------+
|  {.content-wrapper}            | *How can an application use       |
| ![Hybrid Development              | different computing environments  |
| Environment](http://www.c         | during its development, test, and |
| loudcomputingpatterns.org/icons/h | production stages?*               |
| ybrid_development_environment_ico |                                   |
| n.png){.confluence-embedded-image |                                   |
| .confluence-external-resource}    |                                   |
|                                |                                   |
+-----------------------------------+-----------------------------------+
...
### Context[Permalink](http://www.cloudcomputingpatterns.org/hybrid_development_environment/#context "Permalink") {#Cloud-ContextPermalink.67}

Applications have different requirements on the runtime environment
during their development, test, and production phase. During
development, hardware requirements are often uncertain, so hardware
resources should be flexible to extend resources if necessary. During
the test phase, diverse test systems may be needed to verify the proper
functioning of the application on different operating systems or when
being accessed using different client software, i.e., different
browsers. Large numbers of resources may also be required to perform
load tests. During the productive use other factors, such as security
and availability may be of greater importance than resource flexibility.

### Solution[Permalink](http://www.cloudcomputingpatterns.org/hybrid_development_environment/#solution "Permalink") {#Cloud-SolutionPermalink.67}

The production environment of the application is simulated in the
development and test environment through the use of equivalent
addressing, similar mocking data, and equivalent functionality provided
by the environment. Migration of developed applications is ensured
through transformation of application components or compatibility of
runtimes. Some testing resources are provided exclusively in the
development environment to verify the application behavior under
different circumstances.

![Hybrid Development
Environment](http://www.cloudcomputingpatterns.org/sketches/hybrid_development_environment_sketch.png){.confluence-embedded-image
.confluence-external-resource}



 {#expander-1587372581 .expand-container}
 {#expander-control-1587372581 .expand-control}
Hybrid Multimedia Web Application


 {#expander-content-1587372581 .expand-content}
Website content is largely provided from a static environment.
Multimedia files that cannot be cached efficiently are provided from a
large distributed elastic environment for high-performance access.

 {.table-wrap}
+----------------------------------+----------------------------------+
|  {.content-wrapper}           | *How can non-cacheable content   |
| ![Hybrid\_Multimedia\_Web\       | be integrated efficiently in a   |
| _Application](http://www.cloudco | website that is accessed by a    |
| mputingpatterns.org/icons/hybrid | large globally distributed user  |
| _multimedia_web_application_icon | group?*                          |
| .png){.confluence-embedded-image |                                  |
| .confluence-external-resource}   |                                  |
|                               |                                  |
+----------------------------------+----------------------------------+
...
### Context[Permalink](http://www.cloudcomputingpatterns.org/hybrid_multimedia_web_application/#context "Permalink") {#Cloud-ContextPermalink.68}

A [Distributed
Application](http://www.cloudcomputingpatterns.org/distributed_application/) provides
a website accessed by a large globally distributed user group. While
most of the website is comprised of static content, there is also a
significant amount of multimedia content, such as videos or music that
has to be streamed to users.

### Solution[Permalink](http://www.cloudcomputingpatterns.org/hybrid_multimedia_web_application/#solution "Permalink") {#Cloud-SolutionPermalink.68}

Static website content is hosted in a static environment from where it
is accessed by users. The streaming content is provided by an elastic
cloud environment where it is accessed from the application's [User
Interface
Component](http://www.cloudcomputingpatterns.org/user_interface_component/).
The static content is provided to users' client software and in this
static content, the multimedia content is referenced. Retrieval of this
streaming content is often handled directly by the users' browser
software.

![Hybrid\_Multimedia\_Web\_Application](http://www.cloudcomputingpatterns.org/sketches/hybrid_multimedia_web_application_sketch.png){.confluence-embedded-image
.confluence-external-resource}



 {#expander-1607276661 .expand-container}
 {#expander-control-1607276661 .expand-control}
Hybrid Processing


 {#expander-content-1607276661 .expand-content}
Processing functionality that experiences varying workload is hosted in
an elastic cloud while the remainder of an application resides in a
static environment.

 {.table-wrap}
+----------------------------------+----------------------------------+
|  {.content-wrapper}           | *How can Processing Components   |
| ![Hybrid                         | that experiences varying         |
| Processing](ht                   | workload be hosted in an elastic |
| tp://www.cloudcomputingpatterns. | cloud while the remainder of an  |
| org/icons/hybrid_processing_icon | application is hosted in a       |
| .png){.confluence-embedded-image | static data center?*             |
| .confluence-external-resource}   |                                  |
|                               |                                  |
+----------------------------------+----------------------------------+
...
### Context[Permalink](http://www.cloudcomputingpatterns.org/hybrid_processing/#context "Permalink") {#Cloud-ContextPermalink.69}

A [Distributed
Application](http://www.cloudcomputingpatterns.org/distributed_application/) provides
processing functionality that experience different workload behavior.
The user group accessing the application is, thus, predictable in size,
but accesses the functions provided by the application differently.
While most of the functions are used equally over time and, therefore,
experience [Static
Workload](http://www.cloudcomputingpatterns.org/static_worklaod/),
some [Processing
Components](http://www.cloudcomputingpatterns.org/processing_component/) experience [Periodic
Workload](http://www.cloudcomputingpatterns.org/periodic_worklaod/), [Unpredictable
Workload](http://www.cloudcomputingpatterns.org/unpredictable_workload/),
or [Continuously Changing
Workload](http://www.cloudcomputingpatterns.org/continuously_changing_workload/).

### Solution[Permalink](http://www.cloudcomputingpatterns.org/hybrid_processing/#solution "Permalink") {#Cloud-SolutionPermalink.69}

The [Processing
Components](http://www.cloudcomputingpatterns.org/processing_component/) experiencing
varying workloads are provisioned in an elastic cloud. [Loose
Coupling](http://www.cloudcomputingpatterns.org/loose_coupling/) is
ensured by exchanging information between the hosting environments
asynchronously via messages.

![Hybrid
Processing](http://www.cloudcomputingpatterns.org/sketches/hybrid_processing_sketch.png){.confluence-embedded-image
.confluence-external-resource}



 {#expander-607926496 .expand-container}
 {#expander-control-607926496 .expand-control}
Hybrid User Interface


 {#expander-content-607926496 .expand-content}
Varying workload from a user group interacting asynchronously with an
application is handled in an elastic environment while the remainder of
an application resides in a static environment.

 {.table-wrap}
+-----------------------------------+-----------------------------------+
|  {.content-wrapper}            | *How can a user interface for     |
| ![Hybrid User                     | asynchronous interaction be       |
| Interface](htt                    | hosted in a cloud while being     |
| p://www.cloudcomputingpatterns.or | integrated with an application    |
| g/icons/hybrid_user_interface_ico | otherwise hosted in a static data |
| n.png){.confluence-embedded-image | center?*                          |
| .confluence-external-resource}    |                                   |
|                                |                                   |
+-----------------------------------+-----------------------------------+


### Context[Permalink](http://www.cloudcomputingpatterns.org/hybrid_user_interface/#context "Permalink") {#Cloud-ContextPermalink.70}

An application serves user groups with different workload behavior. One
user group generates [Static
Workload](http://www.cloudcomputingpatterns.org/static_workload/),
while another user group generates [Periodic
Workload](http://www.cloudcomputingpatterns.org/periodic_workload/), [Once-in-a-lifetime
Workload](http://www.cloudcomputingpatterns.org/once_in_a_lifetime_workload/), [Unpredictable
Workload](http://www.cloudcomputingpatterns.org/unpredictable_workload/),
or [Continuously Changing
Workload](http://www.cloudcomputingpatterns.org/continuously_changing_workload/).
Since the predictability of the user group size and workload behavior
differs, it shall be ensured that unexpected peak workloads do not
affect the performance of the application while each user group is
handled by the most suitable cloud environment.

### Solution[Permalink](http://www.cloudcomputingpatterns.org/hybrid_user_interface/#solution "Permalink") {#Cloud-SolutionPermalink.70}

The [User Interface
Component](http://www.cloudcomputingpatterns.org/user_interface_component/) serving
users generating varying workload is hosted in an elastic cloud
environment. Other application components that are hosted in a static
environment. The user interface deployed in the elastic cloud is
integrated with the remainder of the application in a decoupled fashion
using messaging to ensure [Loose
Coupling](http://www.cloudcomputingpatterns.org/loose_coupling/).

![Hybrid User
Interface](http://www.cloudcomputingpatterns.org/sketches/hybrid_user_interface_sketch.png){.confluence-embedded-image
.confluence-external-resource}



 {#expander-1115919370 .expand-container}
 {#expander-control-1115919370 .expand-control}
Three-Tier Cloud Application


 {#expander-content-1115919370 .expand-content}
The presentation, business logic, and data handling is realized as
separate tiers to scale stateless presentation and compute-intensive
processing independently of the data tier, which is harder to scale and
often handled by the cloud provider.

 {.table-wrap}
+-----------------------------------+-----------------------------------+
|  {.content-wrapper}            | *How can presentation logic,      |
| ![Three-Tier Cloud                | business logic, and data handling |
| Application](http://www           | be decomposed into separate tiers |
| .cloudcomputingpatterns.org/icons | that are scaled independently?*   |
| /three_tier_cloud_application_ico |                                   |
| n.png){.confluence-embedded-image |                                   |
| .confluence-external-resource}    |                                   |
|                                |                                   |
+-----------------------------------+-----------------------------------+
...
### Context[Permalink](http://www.cloudcomputingpatterns.org/three_tier_cloud_application/#context "Permalink") {#Cloud-ContextPermalink.71}

A [Distributed
Application](http://www.cloudcomputingpatterns.org/distributed_application/) is
decomposed into application components to scale individual application
functions independently. There can be many differentiating factors of
application tiers. For example, if [Processing
Components](http://www.cloudcomputingpatterns.org/processing_components/) are
more computation intensive or are used less frequently than [User
Interface
Components](http://www.cloudcomputingpatterns.org/user_interface_components/),
aligning the elastic scaling of these two components by summarizing
their implementation in one tier can be inefficient. This issue arises
every time components experiences different [Application
Workloads](http://www.cloudcomputingpatterns.org/cloud_computing_fundamentals/#application_workloads).
The number of provisioned component instances cannot be aligned well to
the different workloads if they are summarized to coarse grained tiers.

### Solution[Permalink](http://www.cloudcomputingpatterns.org/three_tier_cloud_application/#solution "Permalink") {#Cloud-SolutionPermalink.71}

The application is decomposed into three tiers, where each tier is
elastically scaled independently. The presentation tier is comprised of
a load balancer and an application component that implements
the [Stateless
Component](http://www.cloudcomputingpatterns.org/stateless_component/) pattern
and [User Interface
Component](http://www.cloudcomputingpatterns.org/user_interface_component/) pattern.
The business logic tier is comprised of an application component
implementing the [Stateless
Component](http://www.cloudcomputingpatterns.org/stateless_component/) pattern
in addition to the [Processing
Component](http://www.cloudcomputingpatterns.org/processing_component/) pattern.

![Three-Tier Cloud
Application](http://www.cloudcomputingpatterns.org/sketches/three_tier_cloud_application_sketch.png){.confluence-embedded-image
.confluence-external-resource}



 {#expander-900884092 .expand-container}
 {#expander-control-900884092 .expand-control}
Two-Tier Cloud Application


 {#expander-content-900884092 .expand-content}
Presentation and business logic is bundled to one stateless tier that is
easy to scale. This tier is separated from the data tier that is harder
to scale and often handled by a provider-supplied storage offering.

 {.table-wrap}
+----------------------------------+----------------------------------+
|  {.content-wrapper}           | *How can application             |
| ![Two-Tier Cloud                 | functionality be separated from  |
| Application](http://www.         | data handling to scale them      |
| cloudcomputingpatterns.org/icons | independently?*                  |
| /two_tier_cloud_application_icon |                                  |
| .png){.confluence-embedded-image |                                  |
| .confluence-external-resource}   |                                  |
|                               |                                  |
+----------------------------------+----------------------------------+
...
### Context[Permalink](http://www.cloudcomputingpatterns.org/two_tier_cloud_application/#context "Permalink") {#Cloud-ContextPermalink.72}

A [Distributed
Application](http://www.cloudcomputingpatterns.org/distributed_application/) is
decomposed into application components to scale individual application
functions independently. In this scope, data handling functionality is
significantly harder to scale than [Stateless
Components](http://www.cloudcomputingpatterns.org/stateless_components/),
because [Stateful
Components](http://www.cloudcomputingpatterns.org/stateful_components/) have
to coordinate state information between instances. Therefore, the
application shall be decomposed in a fashion that separates the
easy-to-scale functionality from the hard-to-scale functionality.

### Solution[Permalink](http://www.cloudcomputingpatterns.org/two_tier_cloud_application/#solution "Permalink") {#Cloud-SolutionPermalink.72}

Application functionality is decomposed into data handling
functionality, provided by one or several [Storage
Offerings](http://www.cloudcomputingpatterns.org/cloud_offerings/#storage_offerings),
and application components handling presentation and business logic.
This separation enables the two tiers to elastically scale independently
with their workloads.

![Two-Tier Cloud
Application](http://www.cloudcomputingpatterns.org/sketches/two_tier_cloud_application_sketch.png){.confluence-embedded-image
.confluence-external-resource}
...
\
 ...
 {#expander-697246541 .expand-container}
 {#expander-control-697246541 .expand-control}
References


 {#expander-content-697246541 .expand-content}
...
-   [Cloud Computing
    Patterns](http://www.cloudcomputingpatterns.org/)
-   [Medium faun - scaling applications in the
    cloud](https://medium.com/faun/scaling-applications-in-the-cloud-52bb6dfbac4e)





 {#expander-1007776868 .expand-container}
 {#expander-control-1007776868 .expand-control}
Cloud Design Patterns


 {#expander-content-1007776868 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
Data Management {#Cloud-DataManagement}
---------------
...
Design and Implementation {#Cloud-DesignandImplementation}
-------------------------
...
Management and Monitoring {#Cloud-ManagementandMonitoring}
-------------------------
...
Performance and Scalability {#Cloud-PerformanceandScalability}
---------------------------
...
\
 ...
 {#expander-2046659460 .expand-container}
 {#expander-control-2046659460 .expand-control}
Catalog of Patterns


 {#expander-content-2046659460 .expand-content}
 {.table-wrap}
  Category                      Pattern                                                                                                                                                               Summary
  ----------------------------- --------------------------------------------------------------------------------------------------------------------------------------------------------------------- -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  Availability                  [Geodes](https://wiki.pinnacle.com/display/IT/Geodes)                                                                                                                 Deploy backend services into a set of geographical nodes, each of which can service any client request in any region.
  Availability                  [Health Endpoint Monitoring Pattern](https://wiki.pinnacle.com/display/IT/Health+Endpoint+Monitoring+Pattern)                                                         Implement functional checks in an application that external tools can access through exposed endpoints at regular intervals.
  Availability                  [Queue-Based Load Leveling Pattern](https://wiki.pinnacle.com/display/IT/Queue-Based+Load+Leveling+Pattern)                                                           Use a queue that acts as a buffer between a task and a service that it invokes, to smooth intermittent heavy loads.
  Availability                  [Throttling Pattern](https://wiki.pinnacle.com/display/IT/Throttling+Pattern)                                                                                         Control the consumption of resources by an instance of an application, an individual tenant, or an entire service.
  Data Management               [Cache-Aside Pattern](https://wiki.pinnacle.com/display/IT/Cache-Aside+Pattern)                                                                                       Load data on demand into a cache from a data store
  Data Management               [Command and Query Responsibility Segregation (CQRS) Pattern](https://wiki.pinnacle.com/display/IT/Command+and+Query+Responsibility+Segregation+%28CQRS%29+Pattern)   Segregate operations that read data from operations that update data by using separate interfaces.
  Data Management               [Event Sourcing Pattern](https://wiki.pinnacle.com/display/IT/Event+Sourcing+Pattern)                                                                                 Use an append-only store to record the full series of events that describe actions taken on data in a domain
  Data Management               [Index Table Pattern](https://wiki.pinnacle.com/display/IT/Index+Table+Pattern)                                                                                       Create indexes over the fields in data stores that are frequently referenced by queries.
  Data Management               [Materialized View Pattern](https://wiki.pinnacle.com/display/IT/Materialized+View+Pattern)                                                                           Generate prepopulated views over the data in one or more data stores when the data isn\'t ideally formatted for required query operations.
  Data Management               [Sharding Pattern](https://wiki.pinnacle.com/display/IT/Sharding+Pattern)                                                                                             Divide a data store into a set of horizontal partitions or shards.
  Data Management               [Static Content Hosting Pattern](https://wiki.pinnacle.com/display/IT/Static+Content+Hosting+Pattern)                                                                 Deploy static content to a cloud-based storage service that can deliver them directly to the client.
  Data Management               [Valet Key Pattern](https://wiki.pinnacle.com/display/IT/Valet+Key+Pattern)                                                                                           Use a token or key that provides clients with restricted direct access to a specific resource or service.
  Design and Implementation     [Ambassador Pattern](https://wiki.pinnacle.com/display/IT/Ambassador+Pattern)                                                                                         Create helper services that send network requests on behalf of a consumer service or application.
  Design and Implementation     [Anti-Corruption Layer Pattern](https://wiki.pinnacle.com/display/IT/Anti-Corruption+Layer+Pattern)                                                                   Implement a façade or adapter layer between a modern application and a legacy system.
  Design and Implementation     [Backends for Frontends Pattern](https://wiki.pinnacle.com/display/IT/Backends+for+Frontends+Pattern)                                                                 Create separate backend services to be consumed by specific frontend applications or interfaces.
  Design and Implementation     [Command and Query Responsibility Segregation (CQRS) Pattern](https://wiki.pinnacle.com/display/IT/Command+and+Query+Responsibility+Segregation+%28CQRS%29+Pattern)   Segregate operations that read data from operations that update data by using separate interfaces
  Design and Implementation     [Compute Resource Consolidation Pattern](https://wiki.pinnacle.com/display/IT/Compute+Resource+Consolidation+Pattern)                                                 Consolidate multiple tasks or operations into a single computational unit
  Design and Implementation     [External Configuration Store Pattern](https://wiki.pinnacle.com/display/IT/External+Configuration+Store+Pattern)                                                     Move configuration information out of the application deployment package to a centralized location.
  Design and Implementation     [Gateway Aggregation Pattern](https://wiki.pinnacle.com/display/IT/Gateway+Aggregation+Pattern)                                                                       Use a gateway to aggregate multiple individual requests into a single request.
  Design and Implementation     [Gateway Offloading Pattern](https://wiki.pinnacle.com/display/IT/Gateway+Offloading+Pattern)                                                                         Offload shared or specialized service functionality to a gateway proxy
  Design and Implementation     [Gateway Routing Pattern](https://wiki.pinnacle.com/display/IT/Gateway+Routing+Pattern)                                                                               Route requests to multiple services using a single endpoint.
  Design and Implementation     [Leader Election Pattern](https://wiki.pinnacle.com/display/IT/Leader+Election+Pattern)                                                                               Coordinate the actions performed by a collection of collaborating task instances in a distributed application by electing one instance as the leader that assumes responsibility for managing the other instances.
  Design and Implementation     [Pipes and Filters Pattern](https://wiki.pinnacle.com/display/IT/Pipes+and+Filters+Pattern)                                                                           Break down a task that performs complex processing into a series of separate elements that can be reused.
  Design and Implementation     [Sidecar Pattern](https://wiki.pinnacle.com/display/IT/Sidecar+Pattern)                                                                                               Deploy components of an application into a separate process or container to provide isolation and encapsulation.
  Design and Implementation     [Static Content Hosting Pattern](https://wiki.pinnacle.com/display/IT/Static+Content+Hosting+Pattern)                                                                 Deploy static content to a cloud-based storage service that can deliver them directly to the client.
  Design and Implementation     [Strangler Pattern](https://wiki.pinnacle.com/display/IT/Strangler+Pattern)                                                                                           Incrementally migrate a legacy system by gradually replacing specific pieces of functionality with new applications and services.
  Management and Monitoring     [Ambassador Pattern](https://wiki.pinnacle.com/display/IT/Ambassador+Pattern)                                                                                         Create helper services that send network requests on behalf of a consumer service or application.
  Management and Monitoring     [Anti-Corruption Layer Pattern](https://wiki.pinnacle.com/display/IT/Anti-Corruption+Layer+Pattern)                                                                   Implement a façade or adapter layer between a modern application and a legacy system.
  Management and Monitoring     [External Configuration Store Pattern](https://wiki.pinnacle.com/display/IT/External+Configuration+Store+Pattern)                                                     Move configuration information out of the application deployment package to a centralized location.
  Management and Monitoring     [Gateway Aggregation Pattern](https://wiki.pinnacle.com/display/IT/Gateway+Aggregation+Pattern)                                                                       Use a gateway to aggregate multiple individual requests into a single request.
  Management and Monitoring     [Gateway Offloading Pattern](https://wiki.pinnacle.com/display/IT/Gateway+Offloading+Pattern)                                                                         Offload shared or specialized service functionality to a gateway proxy.
  Management and Monitoring     [Gateway Routing Pattern](https://wiki.pinnacle.com/display/IT/Gateway+Routing+Pattern)                                                                               Route requests to multiple services using a single endpoint.
  Management and Monitoring     [Health Endpoint Monitoring Pattern](https://wiki.pinnacle.com/display/IT/Health+Endpoint+Monitoring+Pattern)                                                         Implement functional checks in an application that external tools can access through exposed endpoints at regular intervals.
  Management and Monitoring     [Sidecar Pattern](https://wiki.pinnacle.com/display/IT/Sidecar+Pattern)                                                                                               Deploy components of an application into a separate process or container to provide isolation and encapsulation.
  Management and Monitoring     [Strangler Pattern](https://wiki.pinnacle.com/display/IT/Strangler+Pattern)                                                                                           Incrementally migrate a legacy system by gradually replacing specific pieces of functionality with new applications and services.
  Messaging                     [Asynchronous Request-Reply Pattern](https://wiki.pinnacle.com/display/IT/Asynchronous+Request-Reply+Pattern)                                                         Decouple backend processing from a frontend host, where backend processing needs to be asynchronous, but the frontend still needs a clear response.
  Messaging                     [Claim-Check Pattern](https://wiki.pinnacle.com/display/IT/Claim-Check+Pattern)                                                                                       Split a large message into a claim check and a payload to avoid overwhelming a message bus.
  Messaging                     [Choreography Pattern](https://wiki.pinnacle.com/display/IT/Choreography+Pattern)                                                                                     Have each component of the system participate in the decision-making process about the workflow of a business transaction, instead of relying on a central point of control.
  Messaging                     [Competing Consumers Pattern](https://wiki.pinnacle.com/display/IT/Competing+Consumers+Pattern)                                                                       Enable multiple concurrent consumers to process messages received on the same messaging channel.
  Messaging                     [Pipes and Filters Pattern](https://wiki.pinnacle.com/display/IT/Pipes+and+Filters+Pattern)                                                                           Break down a task that performs complex processing into a series of separate elements that can be reused.
  Messaging                     [Priority Queue Pattern](https://wiki.pinnacle.com/display/IT/Priority+Queue+Pattern)                                                                                 Prioritize requests sent to services so that requests with a higher priority are received and processed more quickly than those with a lower priority.
  Messaging                     [Publisher-Subscriber Pattern](https://wiki.pinnacle.com/display/IT/Publisher-Subscriber+Pattern)                                                                     Enable an application to announce events to multiple interested consumers asynchronously, without coupling the senders to the receivers.
  Messaging                     [Queue-Based Load Leveling Pattern](https://wiki.pinnacle.com/display/IT/Queue-Based+Load+Leveling+Pattern)                                                           Use a queue that acts as a buffer between a task and a service that it invokes in order to smooth intermittent heavy loads.
  Messaging                     [Scheduler Agent Supervisor Pattern](https://wiki.pinnacle.com/display/IT/Scheduler+Agent+Supervisor+Pattern)                                                         Coordinate a set of actions across a distributed set of services and other remote resources.
  Messaging                     [Sequential Convoy Pattern](https://wiki.pinnacle.com/display/IT/Sequential+Convoy+Pattern)                                                                           Process a set of related messages in a defined order, without blocking processing of other groups of messages.
  Performance and Scalability   [Cache-Aside Pattern](https://wiki.pinnacle.com/display/IT/Cache-Aside+Pattern)                                                                                       Load data on demand into a cache from a data store
  Performance and Scalability   [Choreography Pattern](https://wiki.pinnacle.com/display/IT/Choreography+Pattern)                                                                                     Have each component of the system participate in the decision-making process about the workflow of a business transaction, instead of relying on a central point of control.
  Performance and Scalability   [Command and Query Responsibility Segregation (CQRS) Pattern](https://wiki.pinnacle.com/display/IT/Command+and+Query+Responsibility+Segregation+%28CQRS%29+Pattern)   Segregate operations that read data from operations that update data by using separate interfaces.
  Performance and Scalability   [Event Sourcing Pattern](https://wiki.pinnacle.com/display/IT/Event+Sourcing+Pattern)                                                                                 Use an append-only store to record the full series of events that describe actions taken on data in a domain.
  Performance and Scalability   [Geodes](https://wiki.pinnacle.com/display/IT/Geodes)                                                                                                                 Deploy backend services into a set of geographical nodes, each of which can service any client request in any region.
  Performance and Scalability   [Index Table Pattern](https://wiki.pinnacle.com/display/IT/Index+Table+Pattern)                                                                                       Create indexes over the fields in data stores that are frequently referenced by queries.
  Performance and Scalability   [Materialized View Pattern](https://wiki.pinnacle.com/display/IT/Materialized+View+Pattern)                                                                           Generate prepopulated views over the data in one or more data stores when the data isn\'t ideally formatted for required query operations.
  Performance and Scalability   [Priority Queue Pattern](https://wiki.pinnacle.com/display/IT/Priority+Queue+Pattern)                                                                                 Prioritize requests sent to services so that requests with a higher priority are received and processed more quickly than those with a lower priority.
  Performance and Scalability   [Queue-Based Load Leveling Pattern](https://wiki.pinnacle.com/display/IT/Queue-Based+Load+Leveling+Pattern)                                                           Use a queue that acts as a buffer between a task and a service that it invokes in order to smooth intermittent heavy loads.
  Performance and Scalability   [Sharding Pattern](https://wiki.pinnacle.com/display/IT/Sharding+Pattern)                                                                                             Divide a data store into a set of horizontal partitions or shards.
  Performance and Scalability   [Static Content Hosting Pattern](https://wiki.pinnacle.com/display/IT/Static+Content+Hosting+Pattern)                                                                 Deploy static content to a cloud-based storage service that can deliver them directly to the client.
  Performance and Scalability   [Throttling Pattern](https://wiki.pinnacle.com/display/IT/Throttling+Pattern)                                                                                         Control the consumption of resources used by an instance of an application, an individual tenant, or an entire service.
  Resiliency                    [Bulkhead Pattern](https://wiki.pinnacle.com/display/IT/Bulkhead+Pattern)                                                                                             Isolate elements of an application into pools so that if one fails, the others will continue to function.
  Resiliency                    [Circuit Breaker Pattern](https://wiki.pinnacle.com/display/IT/Circuit+Breaker+Pattern)                                                                               Handle faults that might take a variable amount of time to fix when connecting to a remote service or resource.
  Resiliency                    [Compensating Transaction Pattern](https://wiki.pinnacle.com/display/IT/Compensating+Transaction+Pattern)                                                             Undo the work performed by a series of steps, which together define an eventually consistent operation.
  Resiliency                    [Health Endpoint Monitoring Pattern](https://wiki.pinnacle.com/display/IT/Health+Endpoint+Monitoring+Pattern)                                                         Implement functional checks in an application that external tools can access through exposed endpoints at regular intervals.
  Resiliency                    [Leader Election Pattern](https://wiki.pinnacle.com/display/IT/Leader+Election+Pattern)                                                                               Coordinate the actions performed by a collection of collaborating task instances in a distributed application by electing one instance as the leader that assumes responsibility for managing the other instances.
  Resiliency                    [Queue-Based Load Leveling Pattern](https://wiki.pinnacle.com/display/IT/Queue-Based+Load+Leveling+Pattern)                                                           Use a queue that acts as a buffer between a task and a service that it invokes in order to smooth intermittent heavy loads.
  Resiliency                    [Retry Pattern](https://wiki.pinnacle.com/display/IT/Retry+Pattern)                                                                                                   Enable an application to handle anticipated, temporary failures when it tries to connect to a service or network resource by transparently retrying an operation that\'s previously failed.
  Resiliency                    [Scheduler Agent Supervisor Pattern](https://wiki.pinnacle.com/display/IT/Scheduler+Agent+Supervisor+Pattern)                                                         Coordinate a set of actions across a distributed set of services and other remote resources.
  Security                      [Federated Identity Pattern](https://wiki.pinnacle.com/display/IT/Federated+Identity+Pattern)                                                                         Delegate authentication to an external identity provider.
  Security                      [Gatekeeper Pattern](https://wiki.pinnacle.com/display/IT/Gatekeeper+Pattern)                                                                                         Protect applications and services by using a dedicated host instance that acts as a broker between clients and the application or service, validates and sanitizes requests, and passes requests and data between them.
  Security                      [Valet Key Pattern](https://wiki.pinnacle.com/display/IT/Valet+Key+Pattern)                                                                                           Use a token or key that provides clients with restricted direct access to a specific resource or service.
...
**Design Patterns**
 ...
 {.panelContent}
 {#expander-968575358 .expand-container}
 {#expander-control-968575358 .expand-control}
Infrastructure Design Patterns


 {#expander-content-968575358 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
**Deisgn Patterns**
 ...
 {.panelContent}
 {#expander-320251701 .expand-container}
 {#expander-control-320251701 .expand-control}
Ambassador Pattern


 {#expander-content-320251701 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533288.png){.confluence-embedded-image
...
-   When connectivity features cannot be generalized and require deeper
 ...
 {#expander-1231302559 .expand-container}
 {#expander-control-1231302559 .expand-control}
Anti-Corruption Layer Pattern


 {#expander-content-1231302559 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533415.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
-   When connectivity features cannot be generalized and require deeper
 ...
 {#expander-288362328 .expand-container}
 {#expander-control-288362328 .expand-control}
Asynchronous Request-Reply Pattern


 {#expander-content-288362328 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
-   The network design allows you to open up ports to receive
 ...
 {#expander-1651354281 .expand-container}
 {#expander-control-1651354281 .expand-control}
Backends for Frontends Pattern


 {#expander-content-1651354281 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533295.png){.confluence-embedded-image
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533296.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
-   When only one interface is used to interact with the backend.
 ...
 {#expander-603827573 .expand-container}
 {#expander-control-603827573 .expand-control}
Bulkhead Pattern


 {#expander-content-603827573 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533299.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533300.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
\
 ...
 {#expander-451891217 .expand-container}
 {#expander-control-451891217 .expand-control}
Cache-Aside Pattern


 {#expander-content-451891217 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533303.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
-   For caching session state information in a web application hosted in
 ...
 {#expander-1813491754 .expand-container}
 {#expander-control-1813491754 .expand-control}
Choreography Pattern


 {#expander-content-1813491754 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533306.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533307.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
finished.
 ...
 {#expander-1877660883 .expand-container}
 {#expander-control-1877660883 .expand-control}
Circuit Breaker Pattern


 {#expander-content-1877660883 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533310.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
-   As a substitute for handling exceptions in the business logic of
 ...
 {#expander-933535367 .expand-container}
 {#expander-control-933535367 .expand-control}
Claim-Check Pattern


 {#expander-content-933535367 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533313.jpg){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
is stored in the payload.
 ...
 {#expander-782402207 .expand-container}
 {#expander-control-782402207 .expand-control}
Command and Query Responsibility Segregation (CQRS) Pattern


 {#expander-content-782402207 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533316.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533317.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533318.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
be most valuable.
 ...
 {#expander-576653987 .expand-container}
 {#expander-control-576653987 .expand-control}
Compensating Transaction Pattern


 {#expander-content-576653987 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
compensating transactions.
 ...
 {#expander-2057991626 .expand-container}
 {#expander-control-2057991626 .expand-control}
Competing Consumers Pattern


 {#expander-content-2057991626 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533327.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
> delivers messages in sequence from a producer to a single consumer.
 ...
 {#expander-1205763381 .expand-container}
 {#expander-control-1205763381 .expand-control}
Compute Resource Consolidation Pattern


 {#expander-content-1205763381 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533329.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
run in their own isolated environment, in a separate computational unit.
 ...
 {#expander-1573358499 .expand-container}
 {#expander-control-1573358499 .expand-control}
Event Sourcing Pattern


 {#expander-content-1573358499 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533331.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
-   Systems where there\'s only a very low occurrence of conflicting
 ...
 {#expander-1342835818 .expand-container}
 {#expander-control-1342835818 .expand-control}
External Configuration Store Pattern


 {#expander-content-1342835818 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533333.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
-   As a way to simplify administration of multiple applications, and
 ...
 {#expander-529841966 .expand-container}
 {#expander-control-529841966 .expand-control}
Federated Identity Pattern


 {#expander-content-529841966 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533335.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
-   The application was originally built using a different
 ...
 {#expander-944779392 .expand-container}
 {#expander-control-944779392 .expand-control}
Gatekeeper Pattern


 {#expander-content-944779392 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533346.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
-   Distributed applications where it\'s necessary to perform request
 ...
 {#expander-712104012 .expand-container}
 {#expander-control-712104012 .expand-control}
Gateway Aggregation Pattern


 {#expander-content-712104012 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533348.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533349.png){.confluence-embedded-image
...
height="150"}![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533529.pngg){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border
height="150"}![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533530.png){.confluence-embedded-image
.confluence-content-image-border
...
Using an API gateway has the following benefits {#Cloud-UsinganAPIgatewayhasthefollowingbenefits}
-----------------------------------------------
...
-   The client or application is located near the backend services and
 ...
 {#expander-958826197 .expand-container}
 {#expander-control-958826197 .expand-control}
Gateway Offloading Pattern


 {#expander-content-958826197 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533351.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
services.
 ...
 {#expander-88764297 .expand-container}
 {#expander-control-88764297 .expand-control}
Gateway Routing Pattern


 {#expander-content-88764297 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533353.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
uses only one or two services.
 ...
 {#expander-932009363 .expand-container}
 {#expander-control-932009363 .expand-control}
Geodes


 {#expander-content-932009363 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533355.jpg){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533356.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
-   Simple architectures and requirements, where geo-redundancy and
 ...
 {#expander-1282566108 .expand-container}
 {#expander-control-1282566108 .expand-control}
Health Endpoint Monitoring Pattern


 {#expander-content-1282566108 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533358.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
-   Complementing existing instrumentation in the application, such as
 ...
 {#expander-2029896723 .expand-container}
 {#expander-control-2029896723 .expand-control}
Index Table Pattern


 {#expander-content-2029896723 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533360.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533361.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533362.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533363.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533364.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533365.png){.confluence-embedded-image
height="250"}
...
-   The balance of the data values for a field selected as the secondary
 ...
 {#expander-1061494765 .expand-container}
 {#expander-control-1061494765 .expand-control}
Leader Election Pattern


 {#expander-content-1061494765 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
-   A third-party solution is more appropriate. For example, the
 ...
 {#expander-40294028 .expand-container}
 {#expander-control-40294028 .expand-control}
Materialized View Pattern


 {#expander-content-40294028 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533368.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
-   Consistency is a high priority. The views might not always be fully
 ...
 {#expander-1988542124 .expand-container}
 {#expander-control-1988542124 .expand-control}
Pipes and Filters Pattern


 {#expander-content-1988542124 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533383.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533384.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533385.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
-   The amount of context or state information required by a step makes
 ...
 {#expander-265052859 .expand-container}
 {#expander-control-265052859 .expand-control}
Priority Queue Pattern


 {#expander-content-265052859 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533387.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533388.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
-   Different users or tenants should be served with different priority.
 ...
 {#expander-886774407 .expand-container}
 {#expander-control-886774407 .expand-control}
Publisher-Subscriber Pattern


 {#expander-content-886774407 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533390.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
-   An application requires near real-time interaction with consumers.
 ...
 {#expander-584862606 .expand-container}
 {#expander-control-584862606 .expand-control}
Queue-Based Load Leveling Pattern


 {#expander-content-584862606 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533392.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
the service with minimal latency.
 ...
 {#expander-756801599 .expand-container}
 {#expander-control-756801599 .expand-control}
Retry Pattern


 {#expander-content-756801599 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533394.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
-   As an alternative to addressing scalability issues in a system. If
 ...
 {#expander-1853979242 .expand-container}
 {#expander-control-1853979242 .expand-control}
Scheduler Agent Supervisor Pattern


 {#expander-content-1853979242 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533396.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
services or access remote resources.
 ...
 {#expander-34584717 .expand-container}
 {#expander-control-34584717 .expand-control}
Scheduler Agent Supervisor Pattern


 {#expander-content-34584717 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
services or access remote resources.
 ...
 {#expander-557916900 .expand-container}
 {#expander-control-557916900 .expand-control}
Sequential Convoy Pattern


 {#expander-content-557916900 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533398.png){.confluence-embedded-image
.confluence-content-image-border height="150"}

In the queue, messages for different categories may be interleaved, as
shown in the following diagram:

![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533399.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
\
 ...
 {#expander-2048807228 .expand-container}
 {#expander-control-2048807228 .expand-control}
Sharding Pattern


 {#expander-content-2048807228 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
Sharding Strategies {#Cloud-ShardingStrategies}
-------------------
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533401.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533402.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533403.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
Scaling and data movement operations {#Cloud-Scalinganddatamovementoperations}
------------------------------------
...
Guidance](https://msdn.microsoft.com/library/dn589795.aspx).
 ...
 {#expander-791021597 .expand-container}
 {#expander-control-791021597 .expand-control}
Sidecar Pattern


 {#expander-content-791021597 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533405.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
-   When the service needs to scale differently than or independently
 ...
 {#expander-1010387177 .expand-container}
 {#expander-control-1010387177 .expand-control}
Static Content Hosting Pattern


 {#expander-content-1010387177 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
-   The volume of static content is very small. The overhead of
 ...
 {#expander-2135331119 .expand-container}
 {#expander-control-2135331119 .expand-control}
Strangler Pattern


 {#expander-content-2135331119 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533408.png){.confluence-embedded-image
.confluence-content-image-border height="150"}
...
-   For smaller systems where the complexity of wholesale replacement is
 ...
 {#expander-1601184331 .expand-container}
 {#expander-control-1601184331 .expand-control}
Throttling Pattern


 {#expander-content-1601184331 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533410.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533411.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
-   To help cost-optimize a system by limiting the maximum resource
 ...
 {#expander-2074976803 .expand-container}
 {#expander-control-2074976803 .expand-control}
Valet Key Pattern


 {#expander-content-2074976803 .expand-content}
Author: [Ryan
Kajiura](https://wiki.pinnacle.com/display/~ryank){.confluence-userlink
.user-mention .current-user-mention}
...
![](https://github.com/RyKaj/Documentation/tree/master/InfrastructureArchitecture/attachments/463533413.png){.confluence-embedded-image
.confluence-thumbnail .confluence-content-image-border height="150"}
...
-   If it\'s necessary to limit the size of the data, especially during
 ...
 {#expander-1554349668 .expand-container}
 {#expander-control-1554349668 .expand-control}
References


 {#expander-content-1554349668 .expand-content}
-   [MS Doc  - architecture
    patterns](https://docs.microsoft.com/en-us/azure/architecture/patterns/)
...
\
 ...
 {#expander-599654854 .expand-container}
 {#expander-control-599654854 .expand-control}
References


 {#expander-content-599654854 .expand-content}
-   [MS Doc  - azure architecture
    patterns](https://docs.microsoft.com/en-us/azure/architecture/patterns/)
...
**Training**
 ...
 {.panelContent}
 {#expander-1334142339 .expand-container}
 {#expander-control-1334142339 .expand-control}
eBooks


 {#expander-content-1334142339 .expand-content}
[![](rest/documentConversion/latest/conversion/thumbnail/463534169/1){height="150"}](/download/attachments/463533284/ebook%20-%20The%20Developer%27s%20Guide%20to%20Azure.pdf?version=1&modificationDate=1585167255007&api=v2){.confluence-embedded-file}

[![](rest/documentConversion/latest/conversion/thumbnail/463534171/1){height="150"}](/download/attachments/463533284/Git%20guide-to-the-cloud.pdf?version=1&modificationDate=1585167261247&api=v2){.confluence-embedded-file}

[![](rest/documentConversion/latest/conversion/thumbnail/469106904/1){height="150"}](/download/attachments/463533284/eBook%20-%20Cloud%20Migration%20Essentials.pdf?version=1&modificationDate=1586257867787&api=v2){.confluence-embedded-file}



 {#expander-1995570576 .expand-container}
 {#expander-control-1995570576 .expand-control}
URLs


 {#expander-content-1995570576 .expand-content}
...
------------
 ...
 {.greybox align="left"}
![](images/icons/bullet_blue.gif){width="8" height="8"} [ebook - The
Developer\'s Guide to Azure.pdf](attachments/463533284/463534169.pdf)
(application/pdf)\
![](images/icons/bullet_blue.gif){width="8" height="8"} [Git
guide-to-the-cloud.pdf](attachments/463533284/463534171.pdf)
(application/pdf)\
![](images/icons/bullet_blue.gif){width="8" height="8"} [eBook - Cloud
Migration Essentials.pdf](attachments/463533284/469106904.pdf)
(application/pdf)\
